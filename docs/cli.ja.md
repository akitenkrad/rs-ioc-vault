...
`ioc-vault` は単一バイナリの CLI です．すべてのコマンドで 3 つのグローバルオプションが使えます：`--db <PATH>` でデータベースを指定し（省略時は `~/.ioc-vault/vault.db`），`--config <PATH>` で設定ファイルを指定します（省略時は `~/.ioc-vault/config.toml`）．`--no-defang` は後述の表示時セーフティを無効化します．

## 表示時セーフティ（defang）

脅威インジケータは本質的に危険であり，多くのターミナル・ログビューア・チャットクライアントは URL・IP アドレス・ドメイン・メールアドレスをクリック可能なリンクに変換します．誤クリックやコピー＆ペーストでマルウェアやフィッシングページへ到達してしまうのを防ぐため，**すべての出力がデフォルトで defang されます**：構造を表す文字を無害なプレースホルダに書き換え，リンクとして辿れないようにします．
...
defang はすべての形式に適用されます．`table` ではインジケータの値が書き換えられ，`json` / `jsonl` では**すべての文字列が再帰的に defang されます**——トップレベルの `value` だけでなく，入れ子になったフィード元ペイロード（`raw_data`）や `metadata` も対象とし，ドキュメント内のどこにも生リンクが残らないようにします．数値・真偽値・オブジェクトのキー・タイムスタンプは変更されません．

実際のインジケータをコピーしたい場合や，`jq` など元の文字列を必要とする後段ツールへ `json` / `jsonl` をパイプする場合は `--no-defang` で生の（クリック可能な）値を出力できます：
